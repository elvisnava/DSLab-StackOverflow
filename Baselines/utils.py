...
def mrr_manually(out_probs, grouped_queries, ground_truth, manually):
    """
    ...
    nr_cands_list = np.zeros(len(out_probs))
    for q in np.unique(grouped_queries):
        ...
        # select wether manually
        man = manually[grouped_queries==q]
        assert(len(np.unique(man))==1)
        ...
        nr_cands_list[grouped_queries==q] = len(gt_group)
        if all(man==0):
            summed_score += 1/rank
    assert(not np.any(rank_list==0)) # now rank_list should be filled completely
    return summed_score/len(np.unique(grouped_queries)), rank_list, nr_cands_list
...
def success_at_n_manually(out_probs, grouped_queries, ground_truth,manually, n=5):
    assert(len(out_probs)==len(ground_truth))
    assert(len(out_probs)==len(grouped_queries))
